...
NOTE that the NOMINATIONS table is modified from the original design. I had to add a new column called "Nomination_Number", so that we can store the Movies, and Nominations, where the same movie was nominated in the same year and in the same category.
